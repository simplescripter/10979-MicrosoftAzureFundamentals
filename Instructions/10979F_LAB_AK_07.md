...
1. In the resulting pane, specify the following settings and click **Create storage**:
    ...
    - Resource group: click **Create new**, in the **Required field** text box below, type **10979F0701-LabRG**
...
1. In the Cloud Shell pane, identify the fully qualified domain name of the Azure VM you deployed in the previous exercise by typing the following, and then pressing Enter: 
...
1. The Cloud Shell will display a message informing you that the authenticity of the remote host cannot be established. This is expected. Type **yes** and press the **Enter** key to continue connecting.
...
1. Once you typed in the text, press the **Ctrl+O** key combination and then press the **Enter** key.
...
1. On MIA-CL1, start Microsoft Edge and browse to the port 8080 on the Azure VM using the same URL you used in the previous exercise. Verify that Microsoft Edge displays the initial Wordpress setup page.

    > **Note**: You are able to connect to the Azure VM because the template used to provision its deployment included a network security group with a rule that allows inbound traffic on TCP port 8080. If that was not the case, you would need to add this rule. 
...
1. On the **Resource groups** blade, click **10979F0701-LabRG**.

1. On the **10979F0701-LabRG** blade, click **Delete resource group**.
...
1. Repeat steps 4 to 6 to delete the **10979F0702-LabRG** resource group.
